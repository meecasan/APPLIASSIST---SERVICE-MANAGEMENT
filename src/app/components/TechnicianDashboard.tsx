...
import { servicesAPI, technicianAPI, getCurrentUser } from '../services/api';
...
const transformServiceRequest = (service: any): ServiceRequest => {
  return {
    ...
    name: service.customer_first_name 
      ...
      : 'Customer',
    appliance: service.appliance_type || service.appliance || 'Appliance',
    applianceBrand: service.appliance_brand || 'Not specified',
    issue: service.issue_description || service.problem_description || 'No description provided',
    contact: service.customer_phone || service.contact_number || '',
...
const transformCustomerBookingToRequest = (booking: any): ServiceRequest => {
  return {
    id: booking.id,
    created: booking.createdAt ? new Date(booking.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A',
    createdTime: booking.createdAt ? new Date(booking.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : '12:00 PM',
    scheduledFor: booking.scheduledDate ? new Date(booking.scheduledDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
    scheduledTime: booking.scheduledTime || '12:00',
    status: mapServiceStatus(booking.status || 'Pending'),
    name: booking.contactName || 'Customer',
    appliance: booking.applianceType || 'Appliance',
    applianceBrand: 'Not specified',
    issue: booking.problemDescription || 'No description provided',
    contact: booking.contactNumber || '',
    address: booking.address || '',
    requestId: booking.reference || `REQ-${String(booking.id).padStart(6, '0')}`,
    technicianNotes: '',
  };
};
...
export function TechnicianDashboard({ user, onLogout, customerBookings = [] }: TechnicianDashboardProps) {
  ...
  useEffect(() => {
    const fetchServiceRequests = async () => {
      setLoading(true);
      try {
        const technicianId = Number(user?.id ?? getCurrentUser()?.id ?? 0);
        const services = await technicianAPI.getMyRequests(technicianId);
        const assignedServices = (services || []).filter((service: any) => {
          const currentTechnicianId = service?.technician_id ?? service?.technicianId;
          if (currentTechnicianId == null) {
            return true;
          }
          return String(currentTechnicianId) === String(technicianId);
        });

        let transformedRequests = assignedServices.map(transformServiceRequest);

        if (customerBookings?.length) {
          const matchingBookings = customerBookings.filter((booking: any) => {
            const currentTechnicianId = booking?.technician_id ?? booking?.technicianId ?? booking?.technician?.technician_id ?? booking?.technician?.id;
            if (currentTechnicianId == null) {
              return false;
            }
            return String(currentTechnicianId) === String(technicianId);
          ...
          const mappedBookings = matchingBookings.map(transformCustomerBookingToRequest);
          const existingRequestIds = new Set(transformedRequests.map((request) => request.requestId));

          mappedBookings.forEach((bookingRequest) => {
            if (!existingRequestIds.has(bookingRequest.requestId)) {
              transformedRequests.push(bookingRequest);
            }
          });
        }
        ...
        setServiceRequests([]); // Empty state on error
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [activeTab, customerBookings, user.id]);
