...
import { authAPI, getCurrentUser, getAuthToken, adminAPI, bookingAPI, setCurrentUser, setAuthToken } from './services/api';
import { rtConnect, rtDisconnect } from './services/realtime';

export type UserRole = 'technician' | 'customer' | 'admin' | null;
...
function App() {
  ...
  useEffect(() => {
    ...
    if (token && savedUser) {
      ...
      setCurrentUser({
        id: savedUser.id || savedUser.user_id,
    ...
    const loadBookings = async () => {
      try {
        if (currentUser?.role === 'customer') {
          const customerId = currentUser.id || (getCurrentUser()?.user_id || getCurrentUser()?.id);
          if (customerId) {
        ...
        console.error('Failed to load bookings for customer:', err);
      }
    };
    loadBookings();
  }, [currentUser]);
  ...
  const handleLogin = async (email: string, password: string, role: UserRole) => {
    try {
      ...
      // Ensure localStorage is in sync (persist user and token)
      try {
        setCurrentUser(userData);
        if (response.token) {
          setAuthToken(response.token);
        }
        console.debug('[App] handleLogin -> persisted user & token', { userData, tokenPresent: !!response.token });
      } catch (err) {
        console.warn('[App] handleLogin -> failed to persist user/token', err);
      }
  ...
  // Booking handlers - refresh bookings from backend after a booking completes
  const handleBookingComplete = async (bookingData: any) => {
    try {
      ...
      console.error('Failed to refresh bookings after completion', err);
    }
  };
