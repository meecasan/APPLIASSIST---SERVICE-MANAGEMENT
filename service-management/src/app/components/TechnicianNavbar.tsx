...
import { LayoutDashboard, ClipboardList, Calendar, Briefcase, User, LogOut, Bell } from 'lucide-react';
import { onNotificationNew, offNotificationNew } from '../services/realtime';

interface TechnicianNavbarProps {
  activeTab: 'dashboard' | 'requests' | 'schedule' | 'services' | 'profile';
  onTabChange: (tab: 'dashboard' | 'requests' | 'schedule' | 'services' | 'profile') => void;
  userEmail: string;
  onLogout: () => void;
}

export function TechnicianNavbar({ activeTab, onTabChange, userEmail, onLogout }: TechnicianNavbarProps) {
  ...
  const navItems = [
    ...
    { id: 'services' as const, label: 'Services Offered', icon: Briefcase },
