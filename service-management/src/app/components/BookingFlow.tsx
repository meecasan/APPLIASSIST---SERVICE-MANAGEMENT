...
export default function BookingFlow({ technician, onBack, onBookingComplete, userEmail }: BookingFlowProps) {
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      const customerId = currentUser?.user_id || currentUser?.id;
      if (!customerId) {
        throw new Error('You must be logged in to book a service.');
      }

      if (!technician.id) {
        throw new Error('Technician information is missing. Please go back and select a technician.');
      }

      // --- Field-by-field validation with clear messages ---
      if (!formData.problemDescription.trim()) {
        throw new Error('Please describe the problem with your appliance.');
      }
      if (!formData.preferredDate) {
        throw new Error('Please select a preferred service date.');
      }
      if (!formData.streetAddress.trim() || !formData.city.trim()) {
        throw new Error('Please provide your complete service address (street and city).');
      }

      // Build the service_address string
      ...
      // Explicitly read the booking fields from state so the payload always contains
      // the latest values submitted by the user.
      const scheduled_date = formData.preferredDate.trim();
      const problem_description = formData.problemDescription.trim();
      const appliance_id = formData.service_id;

      // Build payload using the exact snake_case keys expected by the backend.
      const payload = {
        customer_id: customerId,
        technician_id: technician.id,
        appliance_id,
        appliance_type: formData.applianceType || 'General Appliance',
        scheduled_date,
        service_address,
        problem_description,
      ...
      const response = await serviceRequestsAPI.createServiceRequest(payload);

      console.log('Service request created successfully:', response);

      // Set the booking reference from the response
      const ref = response.reference || `BK-${Date.now().toString().slice(-6)}`;
      setBookingReference(ref);

      // Show success toast
      toast.success('Booking confirmed!', {
        description: `Your booking reference is ${ref}. A technician will contact you soon.`,
      });
      ...
      const bookingData = {
        ...
        address: `${formData.streetAddress}, ${formData.barangay || ''} ${formData.city}`,
      ...
      // Call the callback to save booking
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            {currentStep === 1 && (
              <div className="space-y-6">
                ...
                {/* Appliance Type / Service Selection */}
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {technician.services && technician.services.length > 0 ? (
                      technician.services.map((service: any) => (
                        <button
                          ...
                          onClick={() => setFormData({ 
                            ...
                            applianceType: service.service_name || service.category || 'General Service' 
                          })}
                          className={`px-4 py-3 rounded-lg border-2 text-left font-medium transition-all ${
                            formData.service_id === (service.service_id || service.id)
                              ? 'border-[#1E2F4F] bg-[#1E2F4F] text-white shadow-md'
                              : 'border-gray-300 bg-white text-gray-700 hover:border-[#1E2F4F] hover:bg-gray-50'
                          }`}
                        >
                          <div className="font-semibold">{service.service_name || service.category}</div>
                          {service.starting_price && (
                            <div className={`text-xs mt-1 ${formData.service_id === (service.service_id || service.id) ? 'text-gray-300' : 'text-gray-500'}`}>
                      ...
                      <p className="text-sm text-gray-500 col-span-2">This technician has no listed services.</p>
                    )}
                  </div>
                </div>
