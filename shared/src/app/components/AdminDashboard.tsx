...
import { Users, FileText, CheckCircle, XCircle, Clock, UserCheck, Store, Wrench, Eye, Search, X, Download, ChevronLeft, ChevronRight, ShoppingCart } from 'lucide-react';
...
export function AdminDashboard({
  ...
  const [filterType, setFilterType] = useState<'all' | 'shop-owner' | 'technician'>('all');
  ...
  const shopOwnerPending = pendingApplications.filter((app) => app.role === 'shop-owner').length;
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-[#6B7280]" style={{ fontFamily: 'Manrope, sans-serif' }}>
                              {app.role === 'technician' ? 'Technician' : 'Shop Owner'} Application
                            </p>
                          </div>
                        </div>
                        <button
            ...
            <div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm mb-6">
                ...
                <div className="flex items-center space-x-4 mb-4 border-b border-gray-200">
                  ...
                  <button
                    onClick={() => { setFilterType('shop-owner'); setCurrentPage(1); }}
                    className={`px-4 py-2.5 font-medium transition-colors border-b-2 ${
                      filterType === 'shop-owner'
                        ? 'border-[#1E2F4F] text-[#1E2F4F]'
                        : 'border-transparent text-gray-600 hover:text-gray-900'
                    }`}
                    style={{ fontFamily: 'Manrope, sans-serif' }}
                  >
                    Shop Owners ({shopOwnerPending})
                  </button>
                  <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-6">
              {selectedApplication.role === 'shop-owner' ? (
                <>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <p className="text-sm text-gray-600 mb-1" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>Owner Name</p>
                          <p className="text-base text-gray-900 font-medium" style={{ fontFamily: 'Manrope, sans-serif' }}>
                            {selectedApplication.data.ownerName || 'Not provided'}
                          </p>
                        </div>
                        <div>
                    ...
                    <div className="pt-6 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <p className="text-sm text-gray-600 mb-1" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>Business Name</p>
                          <p className="text-base text-gray-900 font-medium" style={{ fontFamily: 'Manrope, sans-serif' }}>
                            {selectedApplication.data.businessName || 'Not provided'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600 mb-1" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>Business Type</p>
                          <p className="text-base text-gray-900 font-medium" style={{ fontFamily: 'Manrope, sans-serif' }}>
                            {selectedApplication.data.businessType || 'Not provided'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600 mb-1" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>Registration Number</p>
                          <p className="text-base text-gray-900 font-medium" style={{ fontFamily: 'Manrope, sans-serif' }}>
                            {selectedApplication.data.registrationNumber || 'Not provided'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600 mb-1" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>TIN</p>
                          <p className="text-base text-gray-900 font-medium" style={{ fontFamily: 'Manrope, sans-serif' }}>
                            {selectedApplication.data.tin || 'Not provided'}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="pt-6 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <p className="text-sm text-gray-600 mb-1" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>Business Address</p>
                          <p className="text-base text-gray-900 font-medium" style={{ fontFamily: 'Manrope, sans-serif' }}>
                            {selectedApplication.data.businessAddress || 'Not provided'}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600 mb-1" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>City</p>
                          <p className="text-base text-gray-900 font-medium" style={{ fontFamily: 'Manrope, sans-serif' }}>
                            {selectedApplication.data.businessCity || 'Not provided'}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Product Categories */}
                    {selectedApplication.data.productCategories && selectedApplication.data.productCategories.length > 0 && (
                      <div className="pt-6 border-t border-gray-200">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4" style={{ fontFamily: 'Poppins, sans-serif' }}>
                          Product Categories
                        </h3>
                        <div className="flex flex-wrap gap-2">
                          {selectedApplication.data.productCategories.map((category: string) => (
                            <span key={category} className="px-3 py-1.5 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                              {category}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div className="flex items-center space-x-3">
                            ...
                            <span className="text-sm text-gray-900" style={{ fontFamily: 'Manrope, sans-serif' }}>
                              Owner ID: {selectedApplication.data.ownerIdFile ? 'Uploaded' : 'Not uploaded'}
                            </span>
                          </div>
                          {selectedApplication.data.ownerIdFile && (
                        ...
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div className="flex items-center space-x-3">
                            <FileText className="w-5 h-5 text-gray-400" />
                            <span className="text-sm text-gray-900" style={{ fontFamily: 'Manrope, sans-serif' }}>
                              Business Permit: {selectedApplication.data.businessPermitFile ? 'Uploaded' : 'Not uploaded'}
                            </span>
                          </div>
                          {selectedApplication.data.businessPermitFile && (
                            <button className="text-sm text-[#1E2F4F] hover:underline flex items-center space-x-1">
                              <Download className="w-4 h-4" />
                              <span>Download</span>
                            </button>
                        ...
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div className="flex items-center space-x-3">
                            <FileText className="w-5 h-5 text-gray-400" />
                            <span className="text-sm text-gray-900" style={{ fontFamily: 'Manrope, sans-serif' }}>
                              BIR Certificate: {selectedApplication.data.birCertFile ? 'Uploaded' : 'Not uploaded'}
                            </span>
                          </div>
                          {selectedApplication.data.birCertFile && (
                            <button className="text-sm text-[#1E2F4F] hover:underline flex items-center space-x-1">
                              <Download className="w-4 h-4" />
                              <span>Download</span>
                            </button>
                    ...
                    <div className="pt-6 border-t border-gray-200">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-base text-gray-900" style={{ fontFamily: 'Manrope, sans-serif' }}>
                          Pending Review
                        </span>
                      </div>
                    </div>
