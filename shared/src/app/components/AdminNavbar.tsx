...
import { LayoutDashboard, FileText, Users, LogOut, Shield, Bell } from 'lucide-react';
import { onNotificationNew, offNotificationNew } from '../services/realtime';

interface AdminNavbarProps {
  activeTab: 'dashboard' | 'applications';
  onTabChange: (tab: 'dashboard' | 'applications') => void;
  userEmail: string;
  onLogout: () => void;
}

export function AdminNavbar({ activeTab, onTabChange, userEmail, onLogout }: AdminNavbarProps) {
    ...
    <div className="w-64 h-screen bg-white border-r border-gray-200 flex flex-col fixed left-0 top-0">
      ...
      <div className="h-20 flex items-center px-6 border-b border-gray-200">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <span className="text-xl font-semibold text-gray-900 block" style={{ fontFamily: 'Poppins, sans-serif' }}>
              APPLIASSIST
            </span>
            <span className="text-xs text-[#6B7280]" style={{ fontFamily: 'Instrument Sans, sans-serif' }}>
              Admin Portal
            </span>
          </div>
        </div>
